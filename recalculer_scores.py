...
SEUIL_CHURN = 0.32
SEUIL_RISQUE_ELEVE = 0.70
SEUIL_RISQUE_MOYEN = 0.32
...
def recalculer_tous_les_scores():
    ...
    print(f"  Risque élevé (>= {SEUIL_RISQUE_ELEVE}): {clients.filter(score_churn__gte=SEUIL_RISQUE_ELEVE).count()}")
    print(
        f"  Risque moyen  (>= {SEUIL_RISQUE_MOYEN} et < {SEUIL_RISQUE_ELEVE}): "
        f"{clients.filter(score_churn__gte=SEUIL_RISQUE_MOYEN, score_churn__lt=SEUIL_RISQUE_ELEVE).count()}"
    )
    print(f"  Risque faible (< {SEUIL_RISQUE_MOYEN}): {clients.filter(score_churn__lt=SEUIL_RISQUE_MOYEN).count()}")
